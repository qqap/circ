...
const defaultViewWebPreferences = {
  nodeIntegration: false, // Ah, its false here, true for the browser window (which is good, but here not)
  nodeIntegrationInSubFrames: true,
  scrollBounce: true,
  safeDialogs: true,
  safeDialogsMessage: 'Prevent this page from creating additional dialogs',
  // Using buildpreload to build all the seperate preload files in the preload folder into one
  preload: __dirname + '/dist/preload.js', // The preload uses ipc.send
...
ipc.on('setView', function (e, args) {
  setView(args.id)
  // args.bounds = { x: 200, y: args.bounds.y, width: args.bounds.width - 200, height: args.bounds.height}
...
ipc.on('setBounds', function (e, args) {
  ...
  // Recapture the page when the view is resized
  // Copy of the code in capturePage
  ...
  view.webContents.capturePage().then(function (img) {
    ...
    img = img.resize({ width: args.bounds.width, height: args.bounds.height })
    mainWindow.webContents.send('captureData', { id: args.id, url: img.toDataURL() })
  })
})
