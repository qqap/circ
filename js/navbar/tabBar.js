...
const tabBar = {
  ...
  updateTab: function (tabId, tabEl = tabBar.getTab(tabId)) {
    ...
    var subtitleEl = tabEl.querySelector('.subtitle')
    // If tabData.url starts with file://, then it's a local file and 
    if (tabData.url.startsWith('file://')) {
      subtitleEl.textContent = "Computer"
    } else {
      // subtitleEl.textContent = urlParser.parse(tabData.url).hostname
      // Remove subdomain and path from url
      subtitleEl.textContent = tabData.url.replace(/^(?:https?:\/\/)?(?:www\.)?/i, "").split('/')[0]
  ...
  initializeTabDragging: function () {
    tabBar.dragulaInstance = dragula([document.getElementById('tabs-inner')], {
      direction: 'vertical',
      // mirrorContainer: document.getElementById('tabs-inner'),

      // moves: function (el, container, handle) {
      //   return handle.classList.contains('tab-item'); // Add this line
      // }
      // slideFactorY: 25
    })
