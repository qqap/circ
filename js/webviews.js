...
function captureCurrentTab (options) {
  ...
  ipc.send('getCapture', {
    ...
    width: Math.round((window.innerWidth - 200) / 10),
...
const webviews = {
  ...
  getViewBounds: function () {
    if (webviews.viewFullscreenMap[webviews.selectedId]) {
      ...
      position.y = 0 // For the edge to edge experience
      position.height = window.innerHeight // more edge to edge
      position.x = 200
      position.width = window.innerWidth - 200
      // position.y += 40 // add 20px for moving down
      // This does it for the webview
