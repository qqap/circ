...
async function afterPackageBuilt (packagePath) {
  ...
  /* create zip files */
  var output = fs.createWriteStream('dist/app/' + 'Circ-v' + version + '-windows' + (packagePath.includes('ia32') ? '-ia32' : '') + '.zip')
  var archive = archiver('zip', {
    zlib: { level: 9 }
  })
  archive.directory(packagePath, 'Circ-v' + version)
  archive.pipe(output)
  ...
  if (!packagePath.includes('ia32')) {
    ...
    const options = {
      src: packagePath,
      dest: 'dist/app/circ-installer-x64',
    ...
    await installer(options)
      .then(function () {
        fs.renameSync('./dist/app/circ-installer-x64/circ-' + version + '-setup.exe', './dist/app/circ-' + version + '-setup.exe')
      })
      .catch(err => {
